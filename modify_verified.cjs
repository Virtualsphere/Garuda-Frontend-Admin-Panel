...
const srcPath = 'src/pages/lands/LandVerificationDashboard.jsx';
const destPath = 'src/pages/lands/VerifiedLandsDashboard.jsx';

let src = fs.readFileSync(srcPath, 'utf8');

// 1. Extract imports from dest, but add missing icons and axios from src
...
// 2. Extract constants from src
const constantsMatch = src.match(/\/\/ Option constants.*?const DOC_TYPES[^;]+;/s);
const constants = constantsMatch ? constantsMatch[0] : '';

// 3. Extract FormCard from src explicitly
const formCard = src.substring(10792, 12167);

// 4. Manual utility extractions
const utilsAndStyles = `// Avatar colors for farmer initials
const AVATAR_COLORS = [
...
const VerifiedLandsDashboard = () => {
  ...
  // Fetch verified lands
  const fetchVerifiedLands = async () => {
    setLoading(true);
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(\`\${API_BASE_URL}/land/pending-final-verification/complete\`, {
        headers: {
          'Authorization': \`Bearer \${token}\`,
          'Content-Type': 'application/json'
        }
      });
      if (!response.ok) throw new Error('Failed to fetch verified lands');
...
// Now extract the inline edit form from src exactly
let renderEditFormCode = src.substring(33170, 84107); // Using indices obtained from script
// Modify the header actions for "Verified Lands"
renderEditFormCode = renderEditFormCode.replace(/<div className="flex items-center gap-3">.*?<\/div>/s, 
`<div className="flex items-center gap-3">
            ...
            <button 
              onClick={saveLandChanges}
              className="px-5 py-2 bg-[#22c55e] text-white rounded-lg text-xs font-bold tracking-wide hover:bg-[#16a34a] transition-colors shadow-lg shadow-green-500/30 flex items-center gap-2"
              disabled={updatingAction !== null}
            >
              {updatingAction === 'save' ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
              SAVE CHANGES
            </button>
          </div>`);

// Change title to "EDIT VERIFIED LAND"
renderEditFormCode = renderEditFormCode.replace(/TECHNICAL VERIFY WORKFLOW/g, "EDIT VERIFIED LAND");

// Construct the rest
const tableListRender = `  // Filter lands by search
        ...
        <div style={styles.statCard}>
          ...
          <div>
            <div style={styles.statValue}>
              {lands.reduce((sum, l) => sum + (l.landDetails?.total_acres || 0), 0).toFixed(1)}
            </div>
...
fs.writeFileSync(destPath, newComponent + tableListRender);
console.log('Done modifying file with exact substring indices!');
